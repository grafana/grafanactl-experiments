...
Source files copied into plugin (originals unchanged):
  .claude/skills/grafanactl/references/query-patterns.md   → debug-with-grafana/references/query-patterns.md
  .claude/skills/grafanactl/references/resource-model.md   → manage-dashboards/references/resource-model.md
```
...
| # | Decision | Rationale |
|---|----------|-----------|
| D1 | CLI flag audit runs first as a dedicated task | NC-007 mandates consistent `--from`/`--to` vs `--start`/`--end` usage. Actual CLI uses `--from`/`--to` (verified in `cmd/grafanactl/query/command.go`). All subsequent content MUST use verified flags. The audit produces a reference table consumed by all authoring tasks. |
...
| D5 | `resource-model.md` is copied into `claude-plugin/skills/manage-dashboards/references/resource-model.md` | The plugin must be self-contained. External relative paths like `../../../.claude/skills/grafanactl/references/resource-model.md` break when the plugin is installed outside the repo (e.g., `~/.claude/plugins/grafanactl/`). Content duplication is intentional and acceptable. T5 includes copying this file. |
| D6 | `query-patterns.md` is copied into `claude-plugin/skills/debug-with-grafana/references/query-patterns.md` | Same rationale as D5. The source file exists at `.claude/skills/grafanactl/references/query-patterns.md`. T3 includes copying this file. |
| D7 | All time-range flags use `--from`/`--to` (not `--start`/`--end`) | Verified against `cmd/grafanactl/query/command.go` lines 40-41. `investigate-alert` SKILL.md uses `--start`/`--end` incorrectly, but fixing that file is out of scope per spec. New files MUST use correct flags per NC-007. |
...
| Concern | Status |
|---------|--------|
| Existing skills (setup-grafanactl, explore-datasources, investigate-alert) | UNCHANGED. No modifications to any existing skill files. |
...
| query-patterns.md and resource-model.md | COPIED into the plugin. Source files at `.claude/skills/grafanactl/references/` are not modified. Plugin copies are independent — future updates to source files require manual re-copy. |
