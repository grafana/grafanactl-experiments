...
Wave 2+3: (all parallel from T1)    |      |      |
  T2: setup-grafanactl       T3:    T4:    T4b:   T5: grafana-debugger stub
...
- Directory tree: `claude-plugin/agents/`, `claude-plugin/skills/setup-grafanactl/references/`, `claude-plugin/skills/explore-datasources/references/`, `claude-plugin/skills/investigate-alert/`
...
- GIVEN the directory tree WHEN I list all paths THEN `claude-plugin/agents/`, `claude-plugin/skills/setup-grafanactl/references/`, `claude-plugin/skills/explore-datasources/references/`, `claude-plugin/skills/investigate-alert/` all exist
...
Write `claude-plugin/skills/setup-grafanactl/SKILL.md` entirely from scratch using `agent-docs/config-system.md` as the authoritative source for all config paths. Follow the `plugin-dev:skill-development` 6-step process and validation checklist.

**Plugin-dev:skill-development guidance (6-step process):**

1. **Understand** -- The skill teaches agents to configure grafanactl for first-time use. Target user intents: "set up grafanactl", "configure Grafana connection", "authenticate with Grafana", "create a grafanactl context".
2. **Plan reusable contents** -- One reference file: `references/configuration.md` (T3). No scripts or assets needed.
3. **Create structure** -- `skills/setup-grafanactl/SKILL.md` + `references/configuration.md`.
4. **Edit** -- Write in this order: references first (T3), then SKILL.md (this task).
   - **YAML frontmatter**: `name` + `description`. Description must be third-person with specific trigger phrases: "Set up and configure grafanactl for connecting to Grafana instances. Use when the user needs to configure authentication, create contexts, troubleshoot connection issues, or set up grafanactl for the first time."
...
Adapt `skills/investigate-alert/SKILL.md` from the existing `.claude/skills/grafana-investigate-alert/SKILL.md`. The source is clean (~135 lines, zero Bug 1-4 patterns), so adaptation is minimal: update frontmatter per `plugin-dev:skill-development` conventions, add a cross-reference to `setup-grafanactl`, and preserve the 4-step workflow.
...
4. **Edit** -- Adapt SKILL.md with these plugin-dev standards:
   ...
   - **Cross-reference**: Add prerequisite note referencing setup-grafanactl skill (e.g., "If grafanactl is not configured, use the setup-grafanactl skill first").
...
- [ ] Cross-reference to setup-grafanactl present
- [ ] Zero Bug 1-4 patterns (verify: no `auth.type`/`auth.token`, no `| grafanactl graph`, no bare `.[]` jq, no `--all-versions`)
- [ ] All grafanactl commands use correct syntax (`grafanactl query -d <uid> -e '<query>' -o json/graph`)

**Procedure:**

1. Read `.claude/skills/grafana-investigate-alert/SKILL.md` (confirmed clean: correct grafanactl commands, no Bug 1-4 patterns).
2. Adapt SKILL.md:
   ...
   - Add cross-reference to `setup-grafanactl` skill in the Prerequisites section (e.g., "If grafanactl is not configured, use the setup-grafanactl skill first").
...
- GIVEN the SKILL.md content WHEN I search for Bug 1-4 patterns (`auth.type`, `| grafanactl graph`, bare `.[]` on datasource list, `--all-versions`) THEN zero matches are found
- GIVEN the SKILL.md content WHEN I search for `grafanactl query` THEN all query examples use `-d <datasource-uid>` and `-o json` or `-o graph` (correct syntax)
...
- **Frontmatter fields:**
  ...
  - `tools`: array of tools the agent needs. For grafanactl: `["Bash", "Read", "Grep"]` (Bash for running grafanactl commands, Read for reading output files, Grep for searching log content).
...
5. Include note: if grafanactl not configured, guide user through setup first.
...
For each skill (`setup-grafanactl`, `explore-datasources`, `investigate-alert`):
...
1. **Bug verification (all 4 bugs across all plugin files):**
   ...
   **Negative constraint verification (spec lines 247-256):**
   - Grep for `grafanactl resources edit` -- expect zero matches.
   - Grep for `GRAFANACTL_AGENT_MODE` -- expect zero matches.
...
2. **Skill reviews (plugin-dev:skill-reviewer 8-step process):**
   - Run the 8-step process on `skills/setup-grafanactl/SKILL.md`. Address any major or critical issues.
...
4. **Plugin loading test:**
   ...
   - Verify all three skills appear in the skill list (setup-grafanactl, explore-datasources, investigate-alert).
...
- GIVEN `skills/setup-grafanactl/SKILL.md` WHEN reviewed by the skill-reviewer 8-step process THEN no major or critical issues reported
