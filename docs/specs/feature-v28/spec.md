...
- **Changes to grafanactl source code** (GRAFANACTL_AGENT_MODE, structured exit codes, etc.) -- Parallel workstream, not a dependency.
...
- FR-002: `claude-plugin/.claude-plugin/plugin.json` MUST contain at minimum: `name` ("grafanactl"), `version` ("0.1.0"), `description`, and `keywords` array including "grafana", "observability", "prometheus", "loki".
...
- FR-004: All config path references in plugin content MUST use the correct schema from `agent-docs/config-system.md`. Specifically:
  - Token: `grafanactl config set contexts.<name>.grafana.token <value>` (NOT `contexts.<name>.auth.type token` / `contexts.<name>.auth.token`)
  - User: `grafanactl config set contexts.<name>.grafana.user <value>` (NOT `contexts.<name>.auth.username`)
  - Org ID: `grafanactl config set contexts.<name>.grafana.org-id <value>` (NOT `contexts.<name>.namespace`)
  - Stack ID: `grafanactl config set contexts.<name>.grafana.stack-id <value>`

- FR-005: All graph output references MUST use `-o graph` as a codec on the `query` command. The plugin MUST NOT contain any reference to `grafanactl graph` as a standalone subcommand or a pipe target (`| grafanactl graph`).
...
### setup-grafanactl Skill

- FR-008: `skills/setup-grafanactl/SKILL.md` MUST include YAML frontmatter with `name` and `description` fields. The `description` MUST mention setup, configuration, authentication, connection, and first-time use.
...
- FR-012: `skills/setup-grafanactl/references/configuration.md` MUST be written from scratch using `agent-docs/config-system.md` as the sole authoritative source. It MUST NOT be adapted from `.claude/skills/grafanactl/references/configuration.md`.
...
- FR-016: `skills/explore-datasources/SKILL.md` MUST add a cross-reference to the `setup-grafanactl` skill.
...
- FR-022: The agent MUST note that if grafanactl is not configured, it should guide the user through setup first.
...
- GIVEN all plugin content files
  WHEN I search for `| grafanactl graph` or `grafanactl graph` as a standalone command
...
- GIVEN `skills/setup-grafanactl/SKILL.md`
...
- GIVEN `skills/explore-datasources/SKILL.md`
  ...
  THEN all four steps, all four examples, troubleshooting entries, and the Advanced Usage section are present. No `| grafanactl graph` references exist. A cross-reference to `setup-grafanactl` is present.
...
- GIVEN `skills/setup-grafanactl/SKILL.md` is complete
...
- DO NOT reference `grafanactl resources edit` (interactive TTY command, unusable by agents).
- DO NOT reference `grafanactl graph` as a standalone subcommand or pipe target.
...
- DO NOT reference `GRAFANACTL_AGENT_MODE` (documented as planned but not yet implemented).
- The `references/configuration.md` MUST NOT be derived from `.claude/skills/grafanactl/references/configuration.md`.
...
| Adapted discover-datasources content contains undiscovered bugs beyond the 4 known ones | Agent follows incorrect commands at runtime | Manually trace every `grafanactl` command in the plugin through the codebase to verify correctness |
...
- [NEEDS VERIFICATION]: Does the `--all-versions` flag actually exist on `grafanactl resources pull`? Run `grafanactl resources pull --help` to verify. If confirmed, add to manage-dashboards skill in Stage 2.
...
- [DESIGN]: What should the `allowed-tools` frontmatter field contain? grafanactl is invoked via Bash, not a named tool. Verify the field's semantics in Claude Code plugin documentation.
