# Extending grafanactl to Support the Entirety of Grafana Cloud
...
- **Plan for Approach C convergence**: after 3-5 providers are stable, add `RegistryIndex.Populate([]Descriptor)` (~20 LOC) and modify `fetch.go`/`push.go` injection points (~50 LOC) so that `grafanactl resources push slo/my-rule` works as a unified alias[6]. The product-specific command groups remain as convenient shortcuts.
...
**Totals**: ~97 resources across 13 product areas[2]. grafanactl currently covers 12 resources (the App Platform row)[2]. Coverage gap: ~88%.
...
Every public interface in grafanactl was classified by k8s coupling level[1]. The full analysis covers 7 interfaces and 5 core types. Here is the summary:
...
CLEAN (no k8s coupling, fully reusable):
  ...
  PushRegistry / PullRegistry
    -> Returns resources.Descriptors (grafanactl type)
...
**1. Auth Problem**: grafanactl's config loading, TLS resolution, and stack-ID auto-discovery live in `internal/` packages[5]. Go's `internal/` visibility rule means external plugin binaries cannot import these packages[5]. Each plugin must re-implement config loading (~200 LOC), or grafanactl must make `internal/config` a public package (a non-trivial API commitment)[5]. The kubectl model "solves" this by inheriting `KUBECONFIG` via environment, but grafanactl's config is more complex (stack-id discovery, org-id resolution, TLS with custom CA)[5].

**2. UX Divergence Problem**: Without shared access to `format.Codec`, `io.Options`, `fail.DetailedError`, and the Options pattern, plugins diverge from grafanactl's output format, error presentation, and flag naming immediately[5]. The gh extension ecosystem exhibits this: extensions have inconsistent `--format`, `--json`, and `--jq` support despite gh's own rich output system[5].
...
Approach B -- making `grafanactl resources push slo/my-rule` route to an SLO REST adapter -- was evaluated and rejected for immediate implementation[6]. Three reasons:
...
**How providers register at compile-time in `cmd/grafanactl/root/command.go`[5]:**
...
Every non-k8s API returns flat JSON[7]. The adapter synthesizes a k8s-like envelope for grafanactl's internal `resources.Resource` type[7]. This envelope is grafanactl's canonical on-disk format and display format -- it does NOT need to match what the API returns.

```
API response (flat):                    grafanactl Resource (synthesized):
...
```
grafanactl push slo/my-slo.yaml
...
- **`Unprepare(resource)`**: Strip server-generated fields before display/diff[3]. Prevents false diffs when comparing local files against remote state. (Analogous to grafanactl's existing `ServerFieldsStripper` processor, but for non-k8s fields.)[3]
...
After 3-5 providers are implemented and their patterns are stable, the following evolution enables `grafanactl resources push slo/my-rule`[6]:
...
**Step 3: `grafanactl resources push slo/...` starts working as an alias[6]**
...
`grafanactl slo push` continues to work as before[6]. The `resources push slo/...` path is an additional entry point, not a replacement[6]. Users who only work with SLOs never need to learn the unified syntax.
...
- Wire provider registration into `cmd/grafanactl/root/command.go` (~10 LOC)
...
- `cmd/grafanactl/slo/` integration wiring (~20 LOC)
...
- Additional commands beyond push/pull:
  - `grafanactl k6 run <test-id>` -- trigger test run
  - `grafanactl k6 runs list <test-id>` -- list test runs
  - `grafanactl k6 schedule set <test-id>` -- set schedule
...
### How `grafanactl config set` Works

```bash
# Set a provider-specific key
grafanactl config set providers.synth.sm_token=eyJr...
...
Context: production
  ...
  synth: ERROR: sm_token not configured
    Set it with: grafanactl config set providers.synth.sm_token=<token>
...
Does the product API use the Grafana k8s /apis endpoint?
  YES -> It is already handled by `grafanactl resources`. No provider needed.
...
Steps:
  ...
  8. Add import + AddCommand in cmd/grafanactl/root/command.go
...
| **SM token lifecycle** -- SM access tokens can expire; provider may fail silently after token rotation | Medium | Medium | Document token refresh procedure. Consider adding `grafanactl synth auth refresh` command. Check token validity in `Validate()` with a lightweight API call. |
| **Config complexity growth** -- 5+ providers each adding 1-3 config keys creates a confusing config file | Medium | Medium | `grafanactl config check` command validates all providers. Clear error messages with "set it with" instructions. `config view` groups by provider with redacted sensitive values. |
...
**1. Should the translation adapter produce the k8s envelope or should grafanactl define a lighter internal representation?[5]**
...
The current design lets each provider define its own command tree via `Commands()`[5]. This maximizes flexibility (k6 can have `run` commands that SLO does not)[5]. But it means no shared `grafanactl push` across all providers[5]. If we later want `grafanactl push ./dir` to automatically dispatch to all configured providers, we would need a shared `PushProvider` sub-interface.
...
**3. How should `grafanactl resources pull` interact with provider resources in a mixed environment?[5]**

Today, `grafanactl resources pull` pulls all app-platform resources[5]. When Approach C lands, should `grafanactl resources pull` also pull SLO/SM/k6 resources by default? Or should it require explicit `grafanactl resources pull --all-providers`?
...
If a user runs `grafanactl slo list` without configuring SLO credentials, should the error come from `Validate()` (before any command runs) or from the HTTP client (401 at runtime)? The Grizzly model uses two-level checks (`Active` vs `Online`)[3]. grafanactl should decide whether `Validate()` is mandatory before command execution or advisory.
...
[1] grafanactl Codebase Analysis: K8s Coupling Interface Boundaries
...
[4] grafanactl CLI Extension Patterns Analysis
    `cmd/grafanactl/root/command.go`, `cmd/grafanactl/resources/command.go`,
    `internal/config/command.go:21-31`, `cmd/grafanactl/fail/detailed.go`
    Conducted 2026-03-02

[5] Plugin/Extension Ecosystem Architecture Decision Analysis for grafanactl
