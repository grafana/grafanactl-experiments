...
| All examples use `grafanactl query -d <uid> -e '<expr>'` syntax | Match actual CLI flag names: `-d` for datasource, `-e` for expression, `--start`/`--end` for time range | Verified against existing `query-patterns.md` and `investigate-alert` SKILL.md | Codebase context |
