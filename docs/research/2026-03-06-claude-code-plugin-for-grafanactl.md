# Research Report: Claude Code Plugin for grafanactl
...
- **grafanactl should become the canonical agent interface for Grafana**[1] by shipping a Claude Code plugin that teaches agents to use the CLI directly, superseding the separate mcp-grafana MCP server.
- **The plugin is skills-first, not MCP-server-first.**[1][2] Unlike the industry pattern where MCP servers call APIs directly, this plugin uses Claude Code's skill system to inject workflow knowledge, and agents invoke grafanactl via the standard Bash tool. This is viable because grafanactl already has comprehensive `-o json` output on every read command.
...
- **Four concrete bugs must be fixed** in existing skill content before any reuse: wrong config paths, fictional `grafanactl graph` pipe command, incorrect JSON envelope paths in jq examples, and an unverified `--all-versions` flag.
...
### The grafanactl Approach (Divergent, But Sound)

The user's vision inverts this: one tool that serves both humans and agents. The plugin teaches agents how to use grafanactl rather than creating a parallel API client.

```
grafanactl Approach:
  Human --> grafanactl CLI --> Grafana K8s API
  Agent --> Skills (teach workflow) --> Bash tool --> grafanactl CLI --> Grafana K8s API
```

This divergence from industry convention is justified by five factors:

1. **grafanactl already outputs structured JSON.**[1] Every read command supports `-o json`, eliminating the primary argument against CLI-wrapping (unparseable output).

2. **grafanactl has unique capabilities mcp-grafana lacks.**[1] SLO management, GitOps push/pull, folder hierarchy sync, resource validation, multi-environment promotion, and the `serve` dev server are all grafanactl-only. Duplicating these in an MCP server means reimplementing `internal/resources/` logic.
...
4. **grafanactl is a compiled Go binary** with sub-100ms startup. Unlike Python or Node CLIs, there is no interpreter overhead penalty for shell invocation.
...
| Capability | mcp-grafana Today | grafanactl Plugin | Gap? |
|-----------|-------------------|-------------------|------|
...
| SLO management | No | Yes | grafanactl ahead |
| GitOps push/pull | No | Yes | grafanactl ahead |
| Resource validation | No | Yes | grafanactl ahead |
| Multi-env promotion | No | Yes | grafanactl ahead |
| Folder hierarchy sync | No | Yes | grafanactl ahead |
| Annotations | Yes | No | Future provider |
| Rendering/screenshots | Yes | No | Out of scope |

The plugin supersedes mcp-grafana for the core observability workflow (debug, explore, instrument, monitor, promote)[1]. Niche mcp-grafana capabilities (incidents, on-call, rendering) are either future grafanactl provider targets or genuinely out of scope.
...
```json
{
  "name": "grafanactl",
  "version": "0.1.0",
  "description": "Debug, explore, and instrument with Grafana using grafanactl CLI",
  "author": {
    "name": "Grafana Labs",
    "url": "https://github.com/grafana"
  },
  "homepage": "https://grafana.com/docs/grafanactl",
  "repository": "https://github.com/grafana/grafanactl-claude-plugin",
  "license": "Apache-2.0",
  "keywords": [
    "grafana",
    "observability",
...
You are a Grafana debugging specialist. You use grafanactl to systematically
diagnose application issues by querying Prometheus metrics, Loki logs, and
SLO status.

Your approach:
1. Discover available datasources (grafanactl datasources list)
...
If grafanactl is not configured, guide the user through setup first.
```
...
| `setup-grafanactl` | "Set up and configure grafanactl for connecting to Grafana instances. Use when the user needs to configure authentication, create contexts, troubleshoot connection issues, or set up grafanactl for the first time." |
...
| `grafanactl graph` pipe patterns | This command does not exist; use `-o graph` on query |
...
2. Prerequisites: grafanactl configured, datasource UIDs known
3. Workflow:
...
- Remove any reference to `grafanactl graph` pipe
...
### Skill 5: setup-grafanactl (Bootstrap Workflow)
...
| setup-grafanactl/SKILL.md | 200 | 4,000 | Setup/config |
...
|-------------|-----------------|--------------|
| `discover-datasources/SKILL.md` | `explore-datasources/SKILL.md` | Remove `grafanactl graph` pipe reference if any; verify no config path references |
...
|-------------|-----------------|-------------|
| `grafanactl/references/query-patterns.md` | `debug-with-grafana/references/query-patterns.md` | Remove all `\| grafanactl graph` pipe examples; replace with `-o graph`. Fix `jq '.[0].uid'` to `jq '.datasources[0].uid'` |
| `grafanactl/references/selectors.md` | `manage-dashboards/references/resource-operations.md` (partial) | Keep syntax table and examples; remove selector resolution algorithm (parse -> discover -> match -> resolve). Remove `--all-versions` flag until verified |
| `grafanactl/references/resource-model.md` | `manage-dashboards/references/resource-model.md` | Keep K8s envelope format, folder ordering, resource kinds. Remove CRUD lifecycle internals and concurrency model details |
...
| Error recovery map | Exists only in codebase analysis, not in any skill | `cmd/grafanactl/fail/convert.go` |
...
| `grafanactl resources edit` | Interactive TTY, unusable by agents |
| `grafanactl graph` as standalone command | Does not exist |
...
**Location**: `grafanactl/SKILL.md` (lines 17-29), `references/configuration.md` (throughout)
...
```bash
grafanactl config set contexts.mystack.grafana.token <api-token>
grafanactl config set contexts.local.grafana.user admin
grafanactl config set contexts.local.grafana.org-id 1
```
...
### Bug 2: `grafanactl graph` Pipe Command Does Not Exist (HIGH)

**Location**: `grafanactl/SKILL.md` (line 72, line 117, line 152), `references/query-patterns.md`

**Wrong**:
```bash
grafanactl query -d <uid> -e 'up' -o json | grafanactl graph
grafanactl query ... -o json | grafanactl graph --title "HTTP Request Rate"
```

**Correct**:
```bash
grafanactl query -d <uid> -e 'up' --start now-1h --end now --step 1m -o graph
```

The graph output is a codec (`-o graph`) on the `query` command, not a standalone subcommand. There is no `grafanactl graph` binary.

### Bug 3: `datasources list` JSON Envelope Path (MEDIUM)

**Location**: `grafanactl/SKILL.md` and `references/query-patterns.md`

**Wrong**:
```bash
grafanactl datasources list -o json | jq '.[0].uid'
```

**Correct**:
```bash
grafanactl datasources list -o json | jq '.datasources[0].uid'
```
...
The flag `grafanactl resources pull dashboards --all-versions` is mentioned but not documented in the CLI layer docs or data-flows docs. This needs verification against the actual command before including in the plugin. Until verified, omit from plugin content.
...
```
grafana/grafanactl-claude-plugin (GitHub repo)
    |
...
# Install
claude plugin install grafanactl@grafana-plugins
```
...
// .claude/settings.json (committed to repo)
{
  ...
  "enabledPlugins": {
    "grafanactl@grafana-plugins": true
  }
}
```
...
```bash
claude plugin install --source github grafana/grafanactl-claude-plugin
```
...
```bash
git clone https://github.com/grafana/grafanactl-claude-plugin ~/.claude/plugins/grafanactl
```
...
| Codex CLI | Ship `AGENTS.md` in grafanactl repo referencing the same workflow docs | Low -- repackage SKILL.md content |
| Gemini CLI | Ship `GEMINI.md` in grafanactl repo | Low -- repackage |
...
grafanactl as the canonical agent interface has these advantages[1]:
...
2. **Richer workflows.** mcp-grafana provides CRUD operations on individual resources. grafanactl provides GitOps workflows (pull entire environment, diff, promote, validate). These multi-step workflows are exactly what agents need for real-world tasks.

3. **Better composability.** grafanactl commands compose naturally in shell pipelines. An agent can chain `datasources list | jq | query | jq` without needing a custom tool for each step.

4. **Already-working auth.** grafanactl's context system (modeled after kubeconfig) handles multi-environment auth natively. mcp-grafana requires separate GRAFANA_URL + GRAFANA_API_KEY configuration.

5. **Extensibility via providers.** New Grafana products (SLO today, Incident/OnCall/Pyroscope tomorrow) plug into grafanactl as providers. Each provider's commands are immediately available to agents through the same CLI. mcp-grafana must add tools individually.

### The Honest Gaps

| mcp-grafana Capability | grafanactl Status | Mitigation |
|------------------------|-------------------|------------|
...
1. **Ship the plugin.** Establish grafanactl as the preferred agent interface for core workflows (debug, explore, instrument, monitor, promote).
2. **Document the overlap.** Show users which mcp-grafana tools map to which grafanactl commands.
3. **Fill capability gaps.** Add incident, oncall, and annotation providers to grafanactl over time.
4. **Deprecate mcp-grafana.** Once grafanactl covers all mcp-grafana capabilities, deprecate the MCP server with a migration guide.
...
|-----------|-------------|
| `hooks/hooks.json` | SessionStart hook to verify grafanactl is installed and configured |
| PostToolUse hook | Auto-suggest `-o json` when agent runs grafanactl without it |
...
| Component | Description |
|-----------|-------------|
| Optional MCP server | `.mcp.json` bundling grafanactl as MCP server for non-skill-aware agents |
| `grafanactl mcp install` | CLI subcommand to auto-configure MCP for all detected agents |
...
| Write setup-grafanactl skill | 0.5 day | From agent-docs/config-system.md |
...
These are not plugin tasks, but improvements to grafanactl that would make the plugin experience better:

| Improvement | Impact on Plugin | Effort |
|-------------|-----------------|--------|
| Ship `AGENTS.md` in grafanactl repo | Enables Codex CLI users to discover grafanactl | Very Low |
| Implement `GRAFANACTL_AGENT_MODE=true` | Auto `-o json`, no color, auto-approve | Medium |
...
**[NEEDS VERIFICATION]**: The planned `GRAFANACTL_AGENT_MODE` environment variable (from design-guide.md) is documented as planned but not implemented. The plugin must not rely on it.
...
**[SPECULATIVE]**: The estimate of 4-5 days for MVP assumes one developer familiar with both grafanactl and Claude Code plugin conventions. A developer new to either may need additional ramp-up time.
...
3. **Incident and OnCall provider roadmap.** These are listed as future grafanactl providers but their timeline and design are unknown. The plugin's positioning against mcp-grafana depends partly on these filling the capability gaps.

4. **Plugin auto-triggering accuracy.** The skill descriptions are designed for auto-triggering, but real-world testing is needed to verify Claude correctly selects the right skill for ambiguous queries (e.g., "show me my Grafana setup" -- is this setup-grafanactl or explore-datasources?).
...
- **Research domains covered**: 3 (Claude Code plugin architecture, CLI agent discoverability patterns, grafanactl codebase surface analysis)
